...
namespace HomeServiceProvider.DataAccess.Repositories.Specific
{
    public class BookingRepository : Repository<Booking>, IBookingRepository
    {
        ...
        // ADD this method inside the existing BookingRepository class
        public async Task<IEnumerable<Booking>> GetDisputedBookingsAsync()
            => await _dbSet
                .Where(b => b.Status == BookingStatus.Disputed)
                .Include(b => b.CustomerProfile).ThenInclude(c => c.User)
                .Include(b => b.ProviderProfile).ThenInclude(p => p.User)
                .Include(b => b.ServiceCategory)
                .Include(b => b.ChatThread).ThenInclude(ct =>
                    ct.Messages.OrderBy(m => m.CreatedAt))
